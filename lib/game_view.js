...
class GameView {
  constructor(renderer, camera, scene, key) {
    ...
    this.lookAroundEl.onclick = this.lookAround.bind(this);
  ...
  lookAround() {
    this.addControls();
    let displayEl = document.getElementsByClassName('game-display')[0];
    displayEl.className = "game-display hidden";
  }
  ...
  addMovingNotes(noteInterval) {
    ...
    songNotes.forEach((songNote, idx) => {
      ...
      // POSITION & ADD TO SCENE NOTES & HOLDS
      setTimeout( () => {
